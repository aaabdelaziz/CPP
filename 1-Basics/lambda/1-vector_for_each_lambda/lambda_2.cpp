...
int main()
{
    ...
    std::for_each(values.begin(), values.end(), [=](int x)
                  { 
        // x= 8; // modifying the input parameter x will cause compilation error as we chose [=] meaning that the input parametes are treated as constants and we can't modify them.
        if(x%2 == 0)
            std::cout << "Number "<< x << " is even" << std::endl;
        else 
            std::cout << "Number " << x << " is odd" << std::endl; });
